"""Shared fixtures for Aave v3 adapter tests."""
...
class FixtureAaveRpcClient(AaveV3RpcClient):
    """Deterministic in-memory Aave RPC fixture client."""
    ...
    def __init__(self, markets_config: MarketsConfig) -> None:
        ...
        for chain_code, chain_config in self._markets.aave_v3.items():
    ...
    def get_block_number(self, chain_code: str) -> int:
        return 22_000_000 + len(chain_code)
    ...
    def get_user_account_data(
        ...
        return UserAccountData(
            ltv_bps=7_500,
            health_factor_wad=int(Decimal("1.75") * Decimal("1e18")),
        )
    ...
    def get_reserve_data(
        ...
        return ReserveData(
            ...
            liquidity_rate_ray=self._to_ray("0.045"),
            variable_borrow_rate_ray=self._to_ray("0.09"),
        )
...
@pytest.fixture()
def fixture_rpc_client(markets_config: MarketsConfig) -> FixtureAaveRpcClient:
    return FixtureAaveRpcClient(markets_config)
...
def build_full_price_map(
    ...
    *,
    ...
    for chain_code, chain_config in markets_config.aave_v3.items():
        for market in chain_config.markets:
